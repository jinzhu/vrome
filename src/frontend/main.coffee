...
isCtrlAcceptKey = (key) ->
  return true  if key is "<C-Enter>"
  false
isAcceptKey = (key) ->
  i = 0

  while i < AcceptKey.length
    return true  if AcceptKey[i] is key
    i++
  false
isEscapeKey = (key) ->
  i = 0

  while i < EscapeKey.length
    return true  if EscapeKey[i] is key
    i++
  false
isCtrlEscapeKey = (key) ->
  ...
  while i < CtrlEscapeKey.length
    return true  if CtrlEscapeKey[i] is key
    i++
  false
AcceptKeyFunction = ->
  Search.next()
...
checkNewMapping = ->
  _.each cmds, (commands, catName) ->
    _.each commands, (info, cmdName) ->
      isFunction = typeof eval_(cmdName) is "function"
      alert cmdName + " is not a function"  unless isFunction
