getKey = (->
  getKey = (evt) ->
    ...
    return (if (ctrl or meta or shift) then ("<" + ctrl + meta + shift + key + ">") else ("<" + key + ">"))  if /^(Enter|Space|BackSpace|Tab|Esc|Home|End|Left|Right|Up|Down|PageUp|PageDown|F(\d\d?))$/.test(key)
    if evt.shiftKey
