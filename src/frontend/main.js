...
(function() {
  var AcceptKey, AcceptKeyFunction, CancelKey, CancelKeyFunction, CtrlEscapeKey, CtrlEscapeKeyFunction, EscapeKey, EscapeKeyFunction, addCmdLineCommands, addErrorLogger, checkNewMapping, cmds, isAcceptKey, isCtrlAcceptKey, isCtrlEscapeKey, isEscapeKey, loadMapping;
  ...
  loadMapping = function() {
    _.each(cmds, function(commands, catName) {
      return _.each(commands, function(info, fname) {
        var func, keys;
  ...
  checkNewMapping = function() {
    return _.each(cmds, function(commands, catName) {
      return _.each(commands, function(info, cmdName) {
        var isFunction;
        isFunction = typeof eval_(cmdName) === "function";
        if (!isFunction) {
          return alert(cmdName + " is not a function");
        }
      });
    });
  };
